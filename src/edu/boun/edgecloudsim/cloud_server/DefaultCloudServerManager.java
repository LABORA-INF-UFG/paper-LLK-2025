...
public class DefaultCloudServerManager extends CloudServerManager{
	...
	public void createVmList(int brockerId){
		...
		for (int i = 0; i < SimSettings.getInstance().getNumOfCloudHost(); i++) {
			...
			for(int j = 0; j < SimSettings.getInstance().getNumOfCloudVMsPerHost(); j++){
				String vmm = "Xen";
				int numOfCores = SimSettings.getInstance().getCoreForCloudVM(j);
				double mips = SimSettings.getInstance().getMipsForCloudVM(j);
				int ram = SimSettings.getInstance().getRamForCloudVM(j);
				long storage = SimSettings.getInstance().getStorageForCloudVM();
				long bandwidth = 0;
				
				//VM Parameters		
				CloudVM vm = new CloudVM(vmCounter, brockerId, mips, numOfCores, ram, bandwidth, storage, vmm, new CustomizedCloudletSchedulerTimeShared());
				vmList.get(i).add(vm);
				vmCounter++;
			}
		}
	...
	//utilization of all VMs per VM
	public String getUtilizationPerVm(){
		...
		for (int hostIndex=0; hostIndex < list.size(); hostIndex++) {
			...
			for(int vmIndex=0; vmIndex<vmArray.size(); vmIndex++){
				totalUtilization += vmArray.get(vmIndex).getCloudletScheduler().getTotalUtilizationOfCpu(CloudSim.clock());
				if (!((hostIndex == list.size() - 1) & (vmIndex == vmArray.size() - 1))){
					totalUtilization = totalUtilization + SimSettings.DELIMITER;
				}
			}
		}
	...
	//average utilization of all VMs
	public double getAvgUtilizationOfRam(){
		double totalUtilization = 0;
		double vmCounter = 0;
		...
		for (int hostIndex=0; hostIndex < list.size(); hostIndex++) {
			...
			for(int vmIndex=0; vmIndex<vmArray.size(); vmIndex++){
				...
				totalUtilization += fuzzyCloudSched.getTotalUtilizationOfRam(CloudSim.clock());
				vmCounter++;
			}
		}
	...
	//utilization of all VMs per VM
	public String getUtilizationPerVmOfRam(){
		String totalUtilization = "";
	...
	private List<Host> createHosts(){
		...
		for (int i = 0; i < SimSettings.getInstance().getNumOfCloudHost(); i++) {
			...
			int numOfCores = SimSettings.getInstance().getCoreForCloudVM(i+3) * numOfVMPerHost;
			double mips = SimSettings.getInstance().getMipsForCloudVM(i+3) * numOfVMPerHost;
			int ram = SimSettings.getInstance().getRamForCloudVM(i+3) * numOfVMPerHost;
