...
import edu.boun.edgecloudsim.edge_client.RamUtilizationModel_Custom;
...
public class LOTOSMobileDeviceManager extends MobileDeviceManager {
	...
	public LOTOSMobileDeviceManager() throws Exception{
	}
	...
	public UtilizationModel getRamUtilizationModel() {
		return new RamUtilizationModel_Custom();
	}
	...
	protected void processCloudletReturn(SimEvent ev) {
		...
		SimLogger.getInstance().taskExecuted(task.getCloudletId(), CloudSim.clock());

		SimLogger.getInstance().timeUpOfVm(task.getVmId(), CloudSim.clock(), task.getAssociatedDatacenterId(), "stop");

		if(task.getAssociatedDatacenterId() == SimSettings.CLOUD_DATACENTER_ID){
			double WanDelay = networkModel.getDownloadDelay(SimSettings.CLOUD_DATACENTER_ID, task.getMobileDeviceId(), task);
			double Wanlinkusage = networkModel.getApUsage(SimSettings.CLOUD_DATACENTER_ID, task.getMobileDeviceId(), task, 2);
			if(WanDelay > 0)
			{
				...
				if(task.getSubmittedLocation().getServingWlanId() == currentLocation.getServingWlanId())
				{
					...
					SimLogger.getInstance().setApUsage(task.getCloudletId(), Wanlinkusage, NETWORK_DELAY_TYPES.WAN_DELAY,2);
			...
			double linkusage = networkModel.getApUsage(task.getAssociatedHostId(), task.getMobileDeviceId(), task, 2);
			...
			if(host.getLocation().getServingWlanId() != task.getSubmittedLocation().getServingWlanId())
			{
				...
				linkusage = networkModel.getApUsage(SimSettings.GENERIC_EDGE_DEVICE_ID, SimSettings.GENERIC_EDGE_DEVICE_ID, task, 2);
			...
			if(delay > 0)
			{
				...
				if(task.getSubmittedLocation().getServingWlanId() == currentLocation.getServingWlanId())
				{
					...
					SimLogger.getInstance().setApUsage(task.getCloudletId(), linkusage, delayType,2);
	...
	protected void processOtherEvent(SimEvent ev) {
		...
		switch (ev.getTag()) {
			...
			{
				((LOTOSExperimentalNetworkModel)networkModel).updateMM1QueeuModel();
			...
			case REQUEST_RECEIVED_BY_EDGE_DEVICE_TO_RELAY_NEIGHBOR:
			{
				...
				double manlinkusage =  networkModel.getApUsage(SimSettings.GENERIC_EDGE_DEVICE_ID, SimSettings.GENERIC_EDGE_DEVICE_ID, task, 1);
				if(manDelay>0){
					...
					SimLogger.getInstance().setApUsage(task.getCloudletId(), manlinkusage, NETWORK_DELAY_TYPES.MAN_DELAY, 1);
			...
			case RESPONSE_RECEIVED_BY_EDGE_DEVICE_TO_RELAY_MOBILE_DEVICE:
			{
				...
				if(delay > 0)
				{
					...
					if(task.getSubmittedLocation().getServingWlanId() == currentLocation.getServingWlanId())
					{
						...
						SimLogger.getInstance().setApUsage(task.getCloudletId(), linkusage, NETWORK_DELAY_TYPES.WLAN_DELAY,2);
	...
	public void submitTask(TaskProperty edgeTask, Vm vm, Integer tipo) {
		...
		if (vm == null){
			...
			SimLogger.getInstance().addLog(task.getMobileDeviceId(),
					...
					edgeTask.getStartTime());
					
			SimLogger.getInstance().rejectedBySolver(task.getCloudletId());
		}else{
			...
			if(nextHopId == SimSettings.CLOUD_DATACENTER_ID){
				...
				linkusage = networkModel.getApUsage(task.getMobileDeviceId(), SimSettings.CLOUD_DATACENTER_ID, task, 1);
				...
				linkusage = networkModel.getApUsage(task.getMobileDeviceId(), SimSettings.GENERIC_EDGE_DEVICE_ID, task, 1);
			...
			if(delay>0){
				...
				if(selectedVM != null){
					...
					if(task.getTaskType() == 0){
						ramCap = selectedVM.getCurrentAllocatedRam();
						CustomizedCloudletSchedulerTimeShared fuzzyCloudSched = (CustomizedCloudletSchedulerTimeShared)selectedVM.getCloudletScheduler();
						utVM = fuzzyCloudSched.getTotalUtilizationOfRam(CloudSim.clock());
						if (utVM == 0){
							utVM += 1300 + 2400;
						}else{
							utVM += 2400;
					...
					if(utVM > ramCap){
						SimLogger.getInstance().errorDueToRamCapacity(task.getCloudletId());
					}else{
						//set related host id
						...
						SimLogger.getInstance().setApUsage(task.getCloudletId(), linkusage, delayType, 1);
	...
	public Decision submitTasks(List<TaskProperty> edgeTask) {
		int vmType=0;
		int nextEvent=0;
		int nextDeviceForNetworkModel;
		NETWORK_DELAY_TYPES delayType;
		...
		for(int i=0; i< edgeTask.size(); i++){
			TaskProperty task = edgeTask.get(i);
			tasks.add(task);
		}
		Decision nextHopId = SimManager.getInstance().getEdgeOrchestrator().chooseVmToOffload(tasks);
	...
	public void submitTask(TaskProperty edgeTask) {
		...
		if(delay>0){
			...
			if(task.getTaskType() == 0 && selectedVM != null){
				int ramCap = selectedVM.getCurrentAllocatedRam();
				CustomizedCloudletSchedulerTimeShared fuzzyCloudSched = (CustomizedCloudletSchedulerTimeShared)selectedVM.getCloudletScheduler();
				double utVM = fuzzyCloudSched.getTotalUtilizationOfRam(CloudSim.clock());
				if (utVM == 0){
					utVM += 1300 + 2400;
				}else{
					utVM += 2400;
	...
	private void submitTaskToVm(Task task, SimSettings.VM_TYPES vmType) {
		...
		SimLogger.getInstance().timeUpOfVm(task.getVmId(), CloudSim.clock(), task.getAssociatedDatacenterId(), "start");
	}
	
	private Task createTask(TaskProperty edgeTask){
		UtilizationModel utilizationModel = new UtilizationModelFull(); /*UtilizationModelStochastic*/
		UtilizationModel utilizationModelCPU = getCpuUtilizationModel();
		UtilizationModel utilizationModelRAM = getRamUtilizationModel();

		Task task = new Task(edgeTask.getMobileDeviceId(), ++taskIdCounter,
				edgeTask.getLength(), edgeTask.getPesNumber(),
				edgeTask.getInputFileSize(), edgeTask.getOutputFileSize(),
				utilizationModelCPU, utilizationModelRAM, utilizationModel);
		...
		if (utilizationModelRAM instanceof RamUtilizationModel_Custom) {
			((RamUtilizationModel_Custom)utilizationModelRAM).setTask(task);
		}
