		...
		for(int j=SS.getMinNumOfMobileDev(); j<=SS.getMaxNumOfMobileDev(); j+=SS.getMobileDevCounterSize())
		{
			for(int k=0; k<SS.getSimulationScenarios().length; k++)
			{
				for(int i=0; i<SS.getOrchestratorPolicies().length; i++)
				{
					...
					SimLogger.printLine("Duration: " + SS.getSimulationTime()/1000 + " sec (warm up period: "+ SS.getWarmUpPeriod()/1000 +" sec) - #devices: " + j);
					...
					try
					{
						...
						ScenarioFactory sampleFactory = new LOTOSScenarioFactory(j,SS.getSimulationTime(), orchestratorPolicy, simScenario);
