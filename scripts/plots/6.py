...
def plot():
    ...
    plt.title("AVERAGE VM UTILIZATION ON EDGE")

    plt.plot(X_axis, eOPT, "-o", color = '#006bb3', label='LOTOS')
    plt.plot(X_axis, eFuzzy, "-o", color = '#ff5050', label='Fuzzy [Sonmez et al.]')
    plt.plot(X_axis, eUTIL, "-v", color = '#009933', label='UTIL.')
    plt.plot(X_axis, eCOST, "-^", color = '#001C4F', label='MIN. COST')
    ...
    plt.savefig("graficos/PNG/6.png", dpi=300, bbox_inches='tight')
    plt.savefig("graficos/6.pdf", dpi=100, bbox_inches='tight')
